...
$getReceiverStockQuery = "SELECT elektrik,para FROM kullanicilar WHERE id = $receiverUserId";
$resultReceiver = $conn->query($getReceiverStockQuery);

if ($resultReceiver->num_rows > 0) {
    $rowReceiver = $resultReceiver->fetch_assoc();
    $currentReceiverStock = $rowReceiver["elektrik"];
...
$senderUserId = 1;
$getSenderStockQuery = "SELECT elektrik FROM kullanicilar WHERE id = $senderUserId";
$resultSender = $conn->query($getSenderStockQuery);

if ($resultSender->num_rows > 0) {
    $rowSender = $resultSender->fetch_assoc();
    $currentSenderStock = $rowSender["elektrik"];
} else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $unitPrice =3; // Her bir ürün için fiyat
    ...
    if ($currentSenderStock >= $quantity) {
        ...
        if ($currentReceiverMoney >= $requiredMoney) {
            ...
            // Stokları güncelle
            $updateSenderStockQuery = "UPDATE kullanicilar SET elektrik = $newSenderStock WHERE id = $senderUserId";
            $updateReceiverStockQuery = "UPDATE kullanicilar SET elektrik = $newReceiverStock WHERE id = $receiverUserId";
...
<head>
	...
	<style>
	  body {
            background-image: url('https://cdn.pixabay.com/photo/2014/06/21/20/11/power-station-374097_1280.jpg'); /* Arkaplan resminin dosya adını buraya ekleyin */
...
<div class="container mt-5">
    <div class="card col-md-6 mx-auto">
        <div class="card-body"> 
            ...
            <h2 class="card-title">Elektrik 💡</h2>
   <p class="price-label">Fiyat:💵3</p>
            <div class="user-info">
