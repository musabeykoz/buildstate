...
$getReceiverStockQuery = "SELECT su, para FROM kullanicilar WHERE id = $receiverUserId";
$resultReceiver = $conn->query($getReceiverStockQuery);

if ($resultReceiver->num_rows > 0) {
    $rowReceiver = $resultReceiver->fetch_assoc();
    $currentReceiverStock = $rowReceiver["su"];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $unitPrice = 5; // Her bir ürün için fiyat
    ...
    if ($currentSenderStock >= $quantity) {
        ...
        if ($currentReceiverMoney >= $requiredMoney) {
            ...
            // Stokları güncelle
            $updateSenderStockQuery = "UPDATE kullanicilar SET su = $newSenderStock WHERE id = $senderUserId";
            $updateReceiverStockQuery = "UPDATE kullanicilar SET su = $newReceiverStock WHERE id = $receiverUserId";
...
<head>
	...
	<style>
	  body {
            background-image: url('https://cdn.pixabay.com/photo/2014/12/24/05/02/drop-of-water-578897_1280.jpg'); /* Arkaplan resminin dosya adını buraya ekleyin */
...
<div class="container mt-5">
    <div class="card col-md-6 mx-auto">
        <div class="card-body"> 
            ...
            <h2 class="card-title">Su 💧 </h2>
   <p class="price-label">Fiyat:💵5</p>
            <div class="user-info">
