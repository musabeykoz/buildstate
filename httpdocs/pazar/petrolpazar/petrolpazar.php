...
$getReceiverStockQuery = "SELECT petrol,para FROM kullanicilar WHERE id = $receiverUserId";
$resultReceiver = $conn->query($getReceiverStockQuery);

if ($resultReceiver->num_rows > 0) {
    $rowReceiver = $resultReceiver->fetch_assoc();
    $currentReceiverStock = $rowReceiver["petrol"];
...
$senderUserId = 1;
$getSenderStockQuery = "SELECT petrol FROM kullanicilar WHERE id = $senderUserId";
$resultSender = $conn->query($getSenderStockQuery);

if ($resultSender->num_rows > 0) {
    $rowSender = $resultSender->fetch_assoc();
    $currentSenderStock = $rowSender["petrol"];
} else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($currentSenderStock >= $quantity) {
        ...
        if ($currentReceiverMoney >= $requiredMoney) {
            ...
            // Stokları güncelle
            $updateSenderStockQuery = "UPDATE kullanicilar SET petrol = $newSenderStock WHERE id = $senderUserId";
            $updateReceiverStockQuery = "UPDATE kullanicilar SET petrol = $newReceiverStock WHERE id = $receiverUserId";
...
<head>
	...
	<style>
	  body {
            background-image: url('https://cdn.pixabay.com/photo/2020/04/24/12/51/oil-5086759_1280.png'); /* Arkaplan resminin dosya adını buraya ekleyin */
