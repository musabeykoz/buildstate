...
$getReceiverStockQuery = "SELECT bor, para FROM kullanicilar WHERE id = $receiverUserId";
$resultReceiver = $conn->query($getReceiverStockQuery);

if ($resultReceiver->num_rows > 0) {
    $rowReceiver = $resultReceiver->fetch_assoc();
    $currentReceiverStock = $rowReceiver["bor"];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $unitPrice = 30; // Her bir ürün için fiyat
    ...
    if ($currentSenderStock >= $quantity) {
        ...
        if ($currentReceiverMoney >= $requiredMoney) {
            ...
            // Stokları güncelle
            $updateSenderStockQuery = "UPDATE kullanicilar SET bor = $newSenderStock WHERE id = $senderUserId";
            $updateReceiverStockQuery = "UPDATE kullanicilar SET bor = $newReceiverStock WHERE id = $receiverUserId";
...
<head>
	...
	<style>
	  body {
            background-image: url('https://cdn.pixabay.com/photo/2016/04/07/21/34/cave-1314790_1280.jpg'); /* Arkaplan resminin dosya adını buraya ekleyin */
...
<div class="container mt-5">
    <div class="card col-md-6 mx-auto">
        <div class="card-body"> 
            ...
            <h2 class="card-title">Bor ☁️ </h2>
   <p class="price-label">Fiyat:💵30</p>
            <div class="user-info">
