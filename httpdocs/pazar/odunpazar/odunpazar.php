...
$getReceiverStockQuery = "SELECT odun, para FROM kullanicilar WHERE id = $receiverUserId";
$resultReceiver = $conn->query($getReceiverStockQuery);

if ($resultReceiver->num_rows > 0) {
    $rowReceiver = $resultReceiver->fetch_assoc();
    $currentReceiverStock = $rowReceiver["odun"];
...
$senderUserId = 1;
$getSenderStockQuery = "SELECT odun FROM kullanicilar WHERE id = $senderUserId";
$resultSender = $conn->query($getSenderStockQuery);

if ($resultSender->num_rows > 0) {
    $rowSender = $resultSender->fetch_assoc();
    $currentSenderStock = $rowSender["odun"];
} else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $unitPrice = 7; // Her bir ürün için fiyat
    ...
    if ($currentSenderStock >= $quantity) {
        ...
        if ($currentReceiverMoney >= $requiredMoney) {
            ...
            // Stokları güncelle
            $updateSenderStockQuery = "UPDATE kullanicilar SET odun = $newSenderStock WHERE id = $senderUserId";
            $updateReceiverStockQuery = "UPDATE kullanicilar SET odun = $newReceiverStock WHERE id = $receiverUserId";
...
<head>
	...
	<style>
	  body {
            background-image: url('https://cdn.pixabay.com/photo/2021/11/25/16/59/leaves-6824098_1280.png'); /* Arkaplan resminin dosya adını buraya ekleyin */
...
<div class="container mt-5">
    <div class="card col-md-6 mx-auto">
        <div class="card-body"> 
            ...
            <h2 class="card-title">Odun Pazarı 🌳</h2>
   <p class="price-label">Fiyat:💵7</p>
            <div class="user-info">
